...
func (p *Provider) Chat(messages []llm.ChatMessage) (string, error) {
	...
	logger.Info(fmt.Sprintf("moonshot request: %s", string(data)))
	...
	if resp.StatusCode != 200 {
		return "", errors.New(fmt.Sprintf("moonshot api error, status: %d, body: %s", resp.StatusCode, string(respData)))
	}

	logger.Info(fmt.Sprintf("moonshot response: %s", string(respData)))
	...
	if len(result.Choices) > 0 {
		...
		logger.Info(fmt.Sprintf("moonshot reply: %s", reply))
		return reply, nil
	}

	return "", errors.New("moonshot: empty response")
}
