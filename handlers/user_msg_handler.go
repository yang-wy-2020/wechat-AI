...
func (h *UserMessageHandler) ReplyText() error {
	...
	// 白名单检查：如果配置了 WhitelistUsers，则只对名单内用户回复
	cfg := config.LoadConfig()
	if len(cfg.WhitelistUsers) > 0 {
		allowed := false
		for _, name := range cfg.WhitelistUsers {
			if name == h.sender.NickName {
				allowed = true
				break
			}
		}
		if !allowed {
			logger.Info(fmt.Sprintf("user %s not in whitelist, skip reply", h.sender.NickName))
			return nil
		}
	}
	// 1.获取上下文，如果字符串为空不处理
	...
	reply, err = ai.Completions(h.getRequestText())
	if err != nil {
	...
	// 等待回复间隔
	time.Sleep(time.Duration(config.LoadConfig().ReplyInterval))
