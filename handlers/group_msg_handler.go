...
func (g *GroupMessageHandler) ReplyText() error {
	...
	// 群白名单检查：如果配置了 WhitelistGroups，则只对名单内的群自动回复
	cfg := config.LoadConfig()
	if len(cfg.WhitelistGroups) > 0 {
		allowed := false
		for _, grp := range cfg.WhitelistGroups {
			if grp == g.group.NickName {
				allowed = true
				break
			}
		}
		if !allowed {
			logger.Info(fmt.Sprintf("group %s not in whitelist, skip reply", g.group.NickName))
			return nil
		}
	}
	...
	// 3.请求GPT获取回复
	reply, err = ai.Completions(requestText)
	if err != nil {
